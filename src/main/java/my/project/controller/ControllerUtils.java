...
public class ControllerUtils {
    ...
    static StringBuilder getUrlBuilder(List<String> urlArray) {
        ...
        if (urlArray.get(0).contains("Романов") || urlArray.get(0).contains("Ангус") || urlArray.get(0).contains("Дюрок")) {
            stringBuilder.append("?breed=" + urlArray.get(0));
        } else {
            stringBuilder.append("?producers=" + urlArray.get(0).replaceAll("\\s+", "+"));
        }

        for (int i = 1; i < urlArray.size(); i++) {
            if (urlArray.get(i).contains("Романов") || urlArray.get(i).contains("Ангус") || urlArray.get(i).contains("Дюрок")) {
                stringBuilder.append("&breed=" + urlArray.get(i));
            } else {
                stringBuilder.append("&producers=" + urlArray.get(i).replaceAll("\\s+", "+"));
            }
        }
